...
class DQN:
    ...
    def __init__(self, input_sz_, output_sz_, save_path=None, load_path=None):
        ...
        self.X_state = tf.placeholder(tf.float32, shape=[None,self.input_sz])
        self.X_next_state = tf.placeholder(tf.float32, shape=[None,self.input_sz])
    ...
    def model(self, x, name, trainable, reuse):
        ...
        with tf.variable_scope(name) as scope:
            ...
            #     W1 = tf.get_variable("W1", [self.input_sz, units_layer_1], initializer=tf.contrib.layers.xavier_initializer(seed=1))
            #     b1 = tf.get_variable("b1", [units_layer_1], initializer=tf.contrib.layers.xavier_initializer(seed=1))
            #     W2 = tf.get_variable("W2", [units_layer_2, units_layer_1], initializer=tf.contrib.layers.xavier_initializer(seed=1))
            #     b2 = tf.get_variable("b2", [units_layer_2], initializer=tf.contrib.layers.xavier_initializer(seed=1))
            #     W3 = tf.get_variable("W3", [units_layer_2, self.output_sz], initializer=tf.contrib.layers.xavier_initializer(seed=1))
            #     b3 = tf.get_variable("b3", [self.output_sz], initializer=tf.contrib.layers.xavier_initializer(seed=1))
            #
            # # Forward prop
            ...
            #
            hidden = tf.layers.dense(x, units_layer_1, activation=tf.nn.relu, trainable=trainable, name='dense',
                                     reuse=reuse)
            hidden_drop = tf.layers.dropout(hidden, rate=dropout, training=trainable & self.is_training_ph)
            hidden_2 = tf.layers.dense(hidden_drop, units_layer_2, activation=tf.nn.relu, trainable=trainable,
                                       name='dense_1', reuse=reuse)
            hidden_drop_2 = tf.layers.dropout(hidden_2, rate=dropout, training=trainable & self.is_training_ph)
            hidden_3 = tf.layers.dense(hidden_drop_2, units_layer_3, activation=tf.nn.relu, trainable=trainable,
                                       name='dense_2', reuse=reuse)
            hidden_drop_3 = tf.layers.dropout(hidden_3, rate=dropout, training=trainable & self.is_training_ph)
            l3 = tf.squeeze(
                tf.layers.dense(hidden_drop_3, self.output_sz, trainable=trainable, name='dense_3', reuse=reuse))
    ...
    def get_replay(self, batch_size):
        ...
        indices = np.random.permutation(max_len)[:self.batch_size]
        ...
        # re_states =self.states
        # re_rewards = self.rewards
        # re_actions = self.actions
        # re_done = self.is_final_state
        # re_next_state = self.next_states
    ...
    def get_action(self, observation, episode):
        ...
        eps = np.random.randint(self.odds, size=1)[0]
        if eps == 0:
            action = np.random.randint(self.output_sz, size=1)[0]
        #             print('random')
        else:
            action = np.argmax(Qv)  # np.random.choice(range(len(Qv.ravel())), p=Qv.ravel())
        print(Qv, 'observation ', observation, 'action ', action)
        return action
    ...
    def get_next_q_value(self, next_observations, rewards, re_done, actions):
        ...
        max_val = np.max(Qv, axis = 1)  # np.random.choice(range(len(Qv.ravel())), p=Qv.ravel())
        # next_q = np.zeros((len(re_done), self.output_sz))
        # for i in range(len(re_done)):
        #     next_q[i][actions[i]] = rewards[i] + np.array(re_done[i]) * self.gamma * max_val[i]
    ...
    def get_loss_function(self):
        ...
        targets = self.q_target + self.gamma * \
                  tf.gather_nd(self.target_netw, tf.stack(
                      (tf.range(self.batch_size), tf.cast(tf.argmax(self.target_netw, axis=1), tf.int32)), axis=1))

        q_est = tf.gather_nd(self.online_netw, tf.stack((tf.range(self.batch_size), self.next_actions), axis=1))
        error = tf.square( targets - q_est)
        # q_values = self.online_netw * tf.one_hot(self.next_actions, self.output_sz)
        # error = tf.nn.softmax_cross_entropy_with_logits(logits=q_values, labels=self.Y)
        loss = tf.reduce_mean(error)
        self.optimiz = tf.train.AdamOptimizer(self.lr).minimize(loss)

    def train(self):

        re_states, re_rewards, re_actions, re_done, re_next_state = self.get_replay(self.batch_size)
        ...
        self.tf_sess.run(self.optimiz, feed_dict={self.X_state: re_states, self.next_actions: re_actions,self.X_next_state: re_next_state, self.q_target: re_rewards, self.is_training_ph: True})
